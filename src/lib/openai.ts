...
export async function getSchedule(input: string): Promise<Schedule> {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a form that accepts a natural language input and outputs a structured JSON response. The form collects information about a day and time and number of players. You should output the response in this schema\n\nexport interface ScheduleInput {\n    month: number;\n    date: number;\n    after: number;\n    before: number;\n    players: number;\n}\n\nwhere date is the day (0-31)  is the requested date, month is the month (1-12) of the requested date. after is the hour of the day (0-23) that the user would like to play after, before is the hour of the day the user would like to play before.  Players is a number 1-4 of the number of players to request. Default to 4 players if not specified. \n\nThe current date is ${currentDate}\n`,
      },
      {
        role: "user",
        content: input,
